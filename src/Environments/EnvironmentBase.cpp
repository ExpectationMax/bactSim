...
EnvironmentBase::EnvironmentBase(EnvironmentSettings settings) {
    this->init(settings);
}

EnvironmentBase::EnvironmentBase(H5::Group group) {
...
//    group.openAttribute("dt").read(HDF5_GPUTYPE, &envSettings.dt);
    ...
    EnvironmentBase::init(envSettings);
    this->storage.reset(new H5::Group(group));
}


#ifndef NO_GRAPHICS
void EnvironmentBase::setupVisualizationWindow(Window &win) {
...
void EnvironmentBase::visualize(double normalizer) {
    if(numLigands > 1) {
...
array EnvironmentBase::getLigandMapping(std::vector<int> ligandIds) {
...
void EnvironmentBase::init(EnvironmentSettings settings) {
...
void EnvironmentBase::setupStorage(unique_ptr<H5::Group> storage) {
...
void EnvironmentBase::closeStorage() {
    this->storage.reset();
}
